...
-- Durable CPA settlement facts store for mediation gateway/dashboard settlement metrics.

CREATE TABLE IF NOT EXISTS mediation_settlement_conversion_facts (
...
CREATE INDEX IF NOT EXISTS idx_settlement_facts_account_app_time
  ON mediation_settlement_conversion_facts (account_id, app_id, occurred_at DESC);

CREATE INDEX IF NOT EXISTS idx_settlement_facts_request
  ON mediation_settlement_conversion_facts (request_id);

CREATE INDEX IF NOT EXISTS idx_settlement_facts_placement_time
  ON mediation_settlement_conversion_facts (placement_id, occurred_at DESC);
