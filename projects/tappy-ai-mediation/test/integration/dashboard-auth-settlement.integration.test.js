...
test('dashboard auth: login session enforces account scope for settlement aggregates', async () => {
  ...
  try {
    ...
    await seedScopedRevenue(baseUrl, { accountId: 'org_mediation', appId: 'sample-client-app', cpaUsd: 3.2 }, runtimeOrgHeaders)
    ...
    assert.equal(String(me.payload?.user?.accountId || ''), 'org_mediation')
    ...
    assert.equal(accountRows.every((row) => String(row?.accountId || '') === 'org_mediation'), true)
