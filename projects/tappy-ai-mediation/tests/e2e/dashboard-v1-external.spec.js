...
async function issueRuntimeApiKeyHeaders(baseUrl, input = {}, headers = {}) {
  ...
  const appId = String(input.appId || 'mediation-chatbot')
...
test('dashboard v1 external e2e happy path: config -> v2 bid -> events', async () => {
  ...
  try {
    ...
    const keys = await requestJson(
      baseUrl,
      '/api/v1/public/credentials/keys?appId=mediation-chatbot&environment=prod',
    ...
    const config = await requestJson(
      baseUrl,
      '/api/v1/mediation/config?appId=mediation-chatbot&placementId=chat_from_answer_v1&environment=prod&schemaVersion=schema_v1&sdkVersion=1.0.0&requestAt=2026-02-22T00:00:00.000Z',
      { headers: runtimeHeaders },
    )
    ...
    const [decisions, sdkEvents, scopedDecisions, scopedEvents, mismatchScopeDecisions] = await Promise.all([
      ...
      requestJson(
        baseUrl,
        `/api/v1/dashboard/decisions?requestId=${encodeURIComponent(requestId)}&appId=mediation-chatbot&accountId=org_mediation`,
        { headers: dashboardHeaders },
      ),
      requestJson(
        baseUrl,
        `/api/v1/dashboard/events?requestId=${encodeURIComponent(requestId)}&eventType=sdk_event&appId=mediation-chatbot&accountId=org_mediation`,
        { headers: dashboardHeaders },
      ),
      requestJson(
        baseUrl,
        `/api/v1/dashboard/decisions?requestId=${encodeURIComponent(requestId)}&accountId=acct_missing&appId=mediation-chatbot-other`,
        { headers: dashboardHeaders },
      ),
    ])
