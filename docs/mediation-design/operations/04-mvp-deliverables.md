...
3. 外部输入映射与冲突优先级规则（含 B 输入/输出合同 + 六块 required 矩阵 + Canonical 枚举字典 + 字段级冲突裁决引擎 + mappingAudit 快照 + OpenRTB 投影合同：`openrtbProjectionLite/openrtbProjectionVersion/projectionAuditSnapshotLite` + 六块到 `imp/app/device/user/regs/ext` 最小映射矩阵 + `mapped/partial/unmapped` 处置规则 + C 输入合同 + C 执行顺序/短路机制 + C 输出合同 + Policy 原因码体系 + Policy 审计快照 + D 输入合同 + Adapter 注册与能力声明 + request adapt 子合同 + candidate normalize 子合同 + error normalize 体系）。
