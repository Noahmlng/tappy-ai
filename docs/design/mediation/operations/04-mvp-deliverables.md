...
3. 外部输入映射与冲突优先级规则（含 B 输入/输出合同 + 六块 required 矩阵 + Canonical 枚举字典 + 字段级冲突裁决引擎 + mappingAudit 快照 + OpenRTB 投影合同：`openrtbProjectionLite/openrtbProjectionVersion/projectionAuditSnapshotLite` + 六块到 `imp/app/device/user/regs/ext` 最小映射矩阵 + `mapped/partial/unmapped` 处置规则 + 敏感字段脱敏合同：`redactionPolicyLite/redactionSnapshotLite`（字段分级 + `pass/hash/coarsen/drop` 动作 + 先脱敏后审计） + 数值分桶合同：`bucketDictLite/bucketDictVersion/bucketAuditSnapshotLite`（intent/perf/session 分桶边界 + outlier/unknown 策略 + 审计 `rawValue + bucketValue`） + `app_context` canonical 子合同（`language/session_state/device_performance_score/privacy_status` -> semanticSlot，含 required/optional 与缺失/非法动作） + `signal_normalized` 事件合同（`eventKey/eventIdempotencyKey/samplingDecision/samplingRuleVersion` + 基于 `traceKey` 稳定哈希采样 + ACK/重发语义） + C 输入合同（显式 `policySnapshotLite`，对齐 `evaluate(opportunity_v1, policy_snapshot)` 且仅本地快照评估） + C 执行顺序/短路机制 + C 输出合同（显式 `constraintsLite` 标准出口） + Policy 原因码体系 + Policy 审计快照 + D 输入合同 + Adapter 注册与能力声明 + request adapt 子合同 + candidate normalize 子合同 + error normalize 体系）。
4. 两类供给源最小适配合同（adapter 四件事）与编排基线（含一等执行策略合同 `executionStrategyLite`：`strategyType/parallelFanout/strategyTimeoutMs/fallbackPolicy` + D 输入快照合同 `configSnapshotLite`：`configSnapshotId/resolvedConfigRef/configHash/effectiveAt` + D 输出锚点 `auctionDecisionLite`：`served/winner/price/creativeHandle/debugRef` + Route Plan 触发/裁决/短路口径 + 路由审计快照）。
