...
2. `MEDIATION_API_KEY`
3. `APP_ID`
4. `PLACEMENT_ID` (`chat_from_answer_v1` or `chat_intent_recommendation_v1`)
...
## 4. Step 1: Read Placement Config
...
4. 仅当 `HTTP 200 + status=success + message=No bid` 才是正常 no-fill；`4xx` 需要按错误码修复。

## 6. Step 3: Report Event (Attach Impression)
