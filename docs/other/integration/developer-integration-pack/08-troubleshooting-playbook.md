...
| callback auth failure (`401/403`) | scope/auth errors in response | wrong key, expired token, scope mismatch | rotate key/token and align app/account scope (`environment=prod`) | Security + Integrations P1 |
