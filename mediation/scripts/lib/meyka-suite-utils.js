...
export const GATEWAY_ENTRY = path.join(PROJECT_ROOT, 'src', 'devtools', 'mediation', 'mediation-gateway.js')
...
export async function registerDashboardUser(baseUrl, input = {}) {
  ...
  const appId = String(input.appId || 'mediation-chatbot').trim()
...
export async function resolveAuthContext(baseUrl, args = {}, options = {}) {
  ...
  let accountId = explicitAccountId || (allowAutoRegister ? `org_meyka_${fallbackSuffix}` : 'org_mediation')
  let appId = explicitAppId || (allowAutoRegister ? `meyka_app_${fallbackSuffix}` : 'mediation-chatbot')
...
export function createInlineEventPayload(input = {}) {
  return {
    appId: String(input.appId || 'mediation-chatbot').trim(),
